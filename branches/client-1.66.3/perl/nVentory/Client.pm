...
sub _get_ua
{
	...
	if ($login && $login eq 'autoreg')
	{
		...
		unless ($AUTOREG_PW)
		{
			warn "WARNING: autoreg_password not defined in nventory.conf.  Defaulting to 'mypassword'\n" if ($debug);
		}
		$password = ($AUTOREG_PW ? $AUTOREG_PW : 'mypassword');
		warn "* AUTOREG PASSWORD: $password\n" if ($debug);
		if (! -d '/root')
		{
	...
	if ($login)
	{
		...
		if ( ($response->is_redirect) && ( $login ne 'autoreg' ) )
		{
			...
			my $password = $_[3];
		        ...
		        my $login_login_uri = URI->new("$authbase/login/login");
	                $login_login_uri->scheme('https');
			my $host = $login_login_uri->host;
			warn "Attempting to POST to $login_login_uri for autoreg\n" if ($debug);
                        my $response = $ua->request(POST $login_login_uri, {'login' => $login, 'password' => $password});
			unless (($response->is_redirect) && ($response->header('location') =~ /$host\/(accounts.xml)?$/))
			{
		...
		if ( ($response->is_redirect) && ( $login eq 'autoreg' ) )
		{
			...
			# if it's a redirect BUT NOT sso url, then we should follow the redirect and keep trying the post
			...
			if ($response->is_redirect)
			{
				...
				my $location_uri = URI->new($location);
				if ($location_uri->path =~ /login\/login/)
				{
					warn "UA (autoreg) - redirected to /login/login\n" if ($debug);
				        warn "POST to $SERVER/accounts.xml ( ** for user 'autoreg' ** ) was redirected.
                                              \nAuthenticating to local login path: '/login/login'\n" if ($debug);
		                        my $authbase = 'https://' . $location_uri->authority;
					$ua = auth_login_login($ua, $authbase, $login, $password);
				}
				elsif ($location =~ /http.*:\/\/sso.*\/login/) 
				{
					warn "UA (autoreg) - requested redirect to SSO - bypass to /login/login\n" if ($debug);
					my $authbase = "$SERVER";
					$ua = auth_login_login($ua, $authbase, $login, $password);
				}
				else
				{
					warn "UA (autoreg) - 302 - following redirect ($location)\n" if ($debug);
					while ($location_uri->path !~ /login\/login/)
					{ 
						$response = $ua->request(POST $location_uri->path, { 'foo' => 'bar' });
						$location_uri = URI->new($response->header('location'));
					}
		                        my $authbase = 'https://' . $location_uri->authority;
					$ua = auth_login_login($ua, $authbase, $login, $password);
				}
